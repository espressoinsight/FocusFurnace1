...
$(document).ready(function() {
    ...
    // Function to toggle pause/resume and done-task class
    $(document).on('click', '.task', function() {
        const taskId = $(this).attr('id');

        // Toggle done-task class
        $(this).toggleClass('done-task');

        if ($(this).hasClass('done-task')) {
            pauseTimer(taskId); // Pause the timer if task is marked as done
        } else {
            if (taskTimers[taskId].isPaused) {
                startTimer(taskId); // Resume the timer if task is marked as undone
            }
        }

        taskTimers[taskId].isPaused = !taskTimers[taskId].isPaused;
